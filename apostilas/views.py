...
from .models import Apostila, ViewsApostila
...
def adicionar_apostilas(request):
    ...
    # TODO criar as tags
        views_totais = ViewsApostila.objects.filter(apostila__user = request.user).count()
...
def apostila(request, id):
    ...
    views_totais = ViewsApostila.objects.filter(apostila = apostila).count()
    views_unicas = ViewsApostila.objects.filter(apostila = apostila).values('ip').distinct().count()
    view = ViewsApostila(
